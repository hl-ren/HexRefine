import type { Element, LocalRefinement, Point } from "./types.js";
import { mergeCoincidentNodes } from "./mesh.js";
import { add, lerp, mix, scale, sub } from "./vector.js";

export function refineQ1To3x3(vertices: readonly Point[]): LocalRefinement {
...
export function refineQ1To4Q1(vertices: readonly Point[]): LocalRefinement {
  const [x1, x2, x3, x4] = quad(vertices);
  const x5 = lerp(x1, x2, 1 / 3);
  const x6 = lerp(x1, x2, 2 / 3);
...
export function refineQ1To5Q1(vertices: readonly Point[]): LocalRefinement {
  const [x1, x2, x3, x4] = quad(vertices);
  const x5 = lerp(x1, x2, 1 / 3);
  const x6 = lerp(x1, x2, 2 / 3);
...
export function refineHexTo27Hex(vertices: readonly Point[]): LocalRefinement {
  return subdivideHex(vertices, 3, "RefineH1To27H1");
}

export function refineHexTo5Hex(vertices: readonly Point[]): LocalRefinement {
...
export function refineHexTo4Hex(vertices: readonly Point[]): LocalRefinement {
  const [x1, x2, x3, x4, x5, x6, x7, x8] = hex(vertices);
  const y15 = sub(x5, x1);
  const y48 = sub(x8, x4);
  const y26 = sub(x6, x2);
...
export function refineHexTo13Hex(vertices: readonly Point[], mergeTolerance?: number): LocalRefinement {
  const first = refineHexTo4Hex(vertices);
  const nodes: Point[] = [...first.nodes];
  const elements: Element[] = [first.elements[3]!];
...
export function subdivideHex(
  vertices: readonly Point[],
  divisions: number,
  source = `Hex${divisions}x${divisions}x${divisions}`
): LocalRefinement {
  ...
  const index = (i: number, j: number, k: number) =>
    i * (divisions + 1) * (divisions + 1) + j * (divisions + 1) + k + 1;
...
function trilinear(corners: readonly Point[], u: number, v: number, w: number): Point {
  const weights = [
    (1 - u) * (1 - v) * (1 - w),
    u * (1 - v) * (1 - w),
    u * v * (1 - w),
...
function estimateHexTolerance(vertices: readonly Point[]): number {
  const [x1, x2, x3, x4, x5, x6, x7, x8] = hex(vertices);
  const edges = [
    [x1, x2],
    [x2, x3],
...
function hex(vertices: readonly Point[]): [Point, Point, Point, Point, Point, Point, Point, Point] {
...
function quad(vertices: readonly Point[]): [Point, Point, Point, Point] {
