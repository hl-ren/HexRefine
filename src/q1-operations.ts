import type { Element, Mesh, Point } from "./types.js";
...
interface BoundaryEdge extends EdgeOwner {
...
interface TemplateMesh {
  nodes: Point[];
  elements: Element[];
  regionLocalElementIds: Q1TemplateReplacementResult["regionElementIds"];
}

interface Q1TopologicalBlock {
  elementIds: number[];
  dimensions: [number, number];
  bounds: { min: [number, number]; max: [number, number] };
  mapping: Q1BlockMapping;
  radiusScale: number;
  mergeTolerance: number;
}

interface Q1BlockMapping {
  bottom: Point[];
  right: Point[];
  top: Point[];
  left: Point[];
...
const q1LocalEdges: readonly [number, number][] = [
  [0, 1],
  [1, 2],
  [2, 3],
  [3, 0]
...
export function growQ1BoundaryLayer(
  mesh: Mesh,
  selectedElementIds: readonly number[],
  options: Q1BoundaryLayerGrowOptions
): Q1BoundaryLayerGrowResult {
  assertQ1Mesh(mesh);
  ...
  const nodes: Point[] = mesh.nodes.map((point) => [...point]);
  const elements: Element[] = mesh.elements.map((element) => [...element]);
  const generatedElementIds: number[] = [];
  const layerElementIds: number[][] = Array.from({ length: increments }, () => []);
  const offsetNodeIdByKey = new Map<string, number>();

  const incidentEdgesByNodeId = new Map<number, BoundaryEdge[]>();
  ...
  const offsetNodeId = (nodeId: number, layer: number): number => {
    if (layer === 0) {
      return nodeId;
    }
...
function arrangeBoundaryLayerNodes(
  sourceMesh: Mesh,
  grownNodes: Point[],
  boundaryEdges: readonly BoundaryEdge[],
  offsetNodeIdByKey: ReadonlyMap<string, number>,
  increments: number,
  arrangement: number
): void {
...
function boundaryEdgeChains(mesh: Mesh, boundaryEdges: readonly BoundaryEdge[]): number[][] {
  ...
  const breakNodes = new Set<number>();
...
function incidentEdgesAreStraight(mesh: Mesh, nodeId: number, edges: readonly BoundaryEdge[]): boolean {
  if (edges.length !== 2) {
    return false;
  }
...
function directionFromNode(mesh: Mesh, nodeId: number, edge: BoundaryEdge): [number, number] {
...
function arrangedBoundaryParameter(t: number, arrangement: number): number {
...
export function replaceQ1BlockWithRectangleCircleTemplate(
  mesh: Mesh,
  selectedElementIds: readonly number[],
  options: Q1RectangleCircleTemplateOptions = {}
): Q1TemplateReplacementResult {
  assertQ1Mesh(mesh);
  ...
  const templateOptions: Q1RectangleCircleTemplateOptions = { ...options };
  ...
  const transformedTemplateNodes = template.nodes.map((point) =>
    transformTemplatePoint(point, block.mapping, templateOptions.outerCircleRatio ?? 0.55)
  );
  ...
  const mapRegion = (ids: readonly number[]) => ids.map((id) => keptElementCount + id);
  return {
    mesh: merged,
...
function assertQ1Mesh(mesh: Mesh): void {
  if (inferMeshKind(mesh) !== "Q1") {
    throw new Error("operation is only available for 2D Q1 quad meshes");
  }
}

function selectedBoundaryEdges(mesh: Mesh, selected: ReadonlySet<number>): BoundaryEdge[] {
  const ownersByKey = new Map<string, EdgeOwner[]>();
...
function outwardEdgeNormal(mesh: Mesh, edge: EdgeOwner): [number, number] {
  const a = mesh.nodes[edge.nodeIds[0] - 1]!;
  const b = mesh.nodes[edge.nodeIds[1] - 1]!;
  const tangent: [number, number] = [(b[0] ?? 0) - (a[0] ?? 0), (b[1] ?? 0) - (a[1] ?? 0)];
  const length = Math.hypot(tangent[0], tangent[1]);
  if (length <= 1e-12) {
    throw new Error("boundary edge has zero length");
  }
  let normal: [number, number] = [tangent[1] / length, -tangent[0] / length];
  const midpoint: [number, number] = [((a[0] ?? 0) + (b[0] ?? 0)) * 0.5, ((a[1] ?? 0) + (b[1] ?? 0)) * 0.5];
  const center = elementCenter2(mesh, edge.elementId);
  const away: [number, number] = [midpoint[0] - center[0], midpoint[1] - center[1]];
...
function offsetBoundaryPoint(mesh: Mesh, nodeId: number, incidentEdges: readonly BoundaryEdge[], distance: number): Point {
  const point = mesh.nodes[nodeId - 1]!;
...
function intersectOffsetLines(mesh: Mesh, first: BoundaryEdge, second: BoundaryEdge, distance: number): [number, number] | null {
  ...
  const r: [number, number] = [(a1[0] ?? 0) - (a0[0] ?? 0), (a1[1] ?? 0) - (a0[1] ?? 0)];
  const s: [number, number] = [(b1[0] ?? 0) - (b0[0] ?? 0), (b1[1] ?? 0) - (b0[1] ?? 0)];
  const denominator = cross2(r, s);
  if (Math.abs(denominator) <= 1e-12) {
    return null;
  }
  const delta: [number, number] = [(b0[0] ?? 0) - (a0[0] ?? 0), (b0[1] ?? 0) - (a0[1] ?? 0)];
  const t = cross2(delta, s) / denominator;
  return [(a0[0] ?? 0) + r[0] * t, (a0[1] ?? 0) + r[1] * t];
}

function cumulativeLayerDistances(totalHeight: number, increments: number, growthRatio: number): number[] {
...
function resolveQ1SelectedBlock(mesh: Mesh, selectedElementIds: readonly number[], irregularBoundaryBias: number): Q1TopologicalBlock {
  ...
  const center = shiftedRingCenter(
    averagePoints([bottomLeft, bottomRight, topRight, topLeft]),
    axisU,
    axisV,
    { bottom, right, top, left },
    irregularBoundaryBias
  );
  const mapping = {
    bottom,
    right,
...
function selectedBoundaryEdgesForElements(mesh: Mesh, selected: ReadonlySet<number>): BoundaryEdge[] {
  const ownersByKey = new Map<string, EdgeOwner[]>();
...
function orderedBoundaryLoop(boundaryEdges: readonly BoundaryEdge[]): number[] {
  if (boundaryEdges.length === 0) {
    throw new Error("selected cells do not have an exterior boundary");
  }
  const adjacency = new Map<number, number[]>();
  for (const edge of boundaryEdges) {
    for (const [a, b] of [[edge.nodeIds[0], edge.nodeIds[1]], [edge.nodeIds[1], edge.nodeIds[0]]] as const) {
...
function selectedNodeUseCounts(mesh: Mesh, elementIds: readonly number[]): Map<number, number> {
...
function splitLoopAtCorners(loop: readonly number[], corners: ReadonlySet<number>): number[][] {
...
function shiftedRingCenter(
  center: Point,
  axisU: Point,
  axisV: Point,
  sides: Pick<Q1BlockMapping, "bottom" | "right" | "top" | "left">,
  biasStrength: number
): Point {
...
function boundarySideStraightness(points: readonly Point[]): number {
  if (points.length < 2) {
    return 1;
  }
  const chord = Math.hypot(
    (points[points.length - 1]![0] ?? 0) - (points[0]![0] ?? 0),
    (points[points.length - 1]![1] ?? 0) - (points[0]![1] ?? 0)
  );
...
function samplePolylineByIndex(points: readonly Point[], t: number): Point {
  if (points.length === 0) {
...
function createRectangleCircleTemplate(
  dimensions: [number, number],
  options: Q1RectangleCircleTemplateOptions
): TemplateMesh {
  ...
  const regionLocalElementIds = {
    core: [] as number[],
    squareToCircle: [] as number[],
    annulus: [] as number[],
    circleToSquare: [] as number[]
  };

  if (!omitCore) {
    const centralNodeId = (i: number, j: number) => j * (nx + 1) + i + 1;
...
function radialTemplatePoint(
  squarePoint: [number, number],
  layer: number,
  spec: {
    innerSquare: number;
    innerCircle: number;
  ...
  const p1: [number, number] = [spec.innerSquare * squarePoint[0], spec.innerSquare * squarePoint[1]];
  const p2: [number, number] = [spec.innerCircle * direction[0], spec.innerCircle * direction[1]];
  const p3: [number, number] = [spec.outerCircle * direction[0], spec.outerCircle * direction[1]];
  const p4: [number, number] = squarePoint;
  if (layer <= spec.n1) {
...
function rectangleBoundarySamples(nx: number, ny: number): [number, number][] {
...
function transformTemplatePoint(point: Point, mapping: Q1BlockMapping, outerCircleRatio: number): Point {
  const x = point[0] ?? 0;
  const y = point[1] ?? 0;
...
function coonsTemplatePoint(point: Point, mapping: Q1BlockMapping): Point {
...
function affineTemplatePoint(mapping: Q1BlockMapping, x: number, y: number): Point {
  const dimension = Math.max(mapping.center.length, mapping.axisU.length, mapping.axisV.length);
  return Array.from({ length: dimension }, (_, index) =>
    (mapping.center[index] ?? 0)
      + x * (mapping.axisU[index] ?? 0)
      + y * (mapping.axisV[index] ?? 0)
  );
}

function circularToBoundaryBlend(x: number, y: number, outerCircleRatio: number): number {
...
function scaleBoundaryLayerMesh(
  mesh: Mesh,
  mode: "none" | "fit-bounds" | "preserve-area",
  originalBounds: { min: [number, number]; max: [number, number] },
  originalArea: number
): Mesh {
  if (mode === "none") {
    return mesh;
...
function compactMeshWithoutElements(mesh: Mesh, removedElementIds: ReadonlySet<number>): Mesh {
  ...
  const elements = keptSourceElements.map((element) =>
    element.map((nodeId) => {
...
function q1MeshArea(mesh: Mesh): number {
...
function polygonArea2(points: readonly Point[]): number {
  let twiceArea = 0;
...
function meshBounds2(nodes: readonly Point[]): { min: [number, number]; max: [number, number] } {
  if (nodes.length === 0) {
...
function boundsCenter2(bounds: { min: [number, number]; max: [number, number] }): [number, number] {
  return [
...
function elementCenter2(mesh: Mesh, elementId: number): [number, number] {
  const element = mesh.elements[elementId - 1];
  if (!element) {
...
function minQ1EdgeLength(mesh: Mesh): number {
  let min = Infinity;
...
function shiftedPoint(point: Point, normal: [number, number], distance: number): Point {
  const shifted = [...point];
...
function withOriginalTail(original: Point, xy: [number, number]): Point {
...
function ratioInUnit(value: number, name: string): number {
