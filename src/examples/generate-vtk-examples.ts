...
import type { Point, RefinementResult, RefinementRole } from "../types.js";
...
function runQ1CircleTwoStage(): RefinementResult {
  const center = [0.5, 0.5] as const;
  const base = createQ1UnitSquareMesh(20, 20);
  const stage1Ids = selectElementsByPredicate(base, (elementCenterPoint) =>
    distance(elementCenterPoint, center) <= 0.23
  );
  const stage1 = refineByElementIdsWithReport(base, stage1Ids);
  const stage2Ids = selectElementsByPredicate(stage1.mesh, (elementCenterPoint) =>
    distance(elementCenterPoint, center) <= 0.105
  );
  return refineByElementIdsWithReport(stage1.mesh, stage2Ids);
}

function runHexOneEighth(): RefinementResult {
...
function runHexMiddleCube(): RefinementResult {
  const base = createHexUnitCubeMesh(10, 10, 10);
  return refineByBoxWithReport(base, {
    min: [0.3, 0.3, 0.3],
    max: [0.7, 0.7, 0.7]
...
async function writeVtk(fileName: string, result: RefinementResult, title: string): Promise<void> {
  const { mesh } = result;
...
function roleCode(role: RefinementRole): number {
  switch (role) {
    case "selected":
      return 1;
