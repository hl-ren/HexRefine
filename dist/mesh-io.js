...
export function parseMeshJSON(text) {
    return meshFromSerializable(JSON.parse(text));
}
export function parseMeshText(text, fileName = "") {
...
export function parseLegacyVtkMesh(text) {
...
export function parseVtuMesh(text) {
    ...
    for (const pieceMatch of pieceMatches) {
        ...
        for (let index = 0; index < pointCount; index += 1) {
            ...
            const point = pointComponentCount === 2
                ? [
                    pointValues[base],
                    pointValues[base + 1]
                ]
        ...
        const connectivityCount = connectivityCountValue;
...
export function parseAbaqusInpMesh(text) {
    const lines = normalizeImportText(text).split("\n");
    const nodesByExternalId = new Map();
    const elementsByExternalId = [];
    let section = "none";
    let expectedNodeCount;
    let pendingElementFields = [];
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (section === "element") {
            ...
            const resolvedNodeCount = expectedNodeCount ?? inferConnectivityNodeCountFromFieldCount(pendingElementFields.length, "Abaqus *Element");
            ...
            parseExternalElementRecord(pendingElementFields, resolvedNodeCount, `Abaqus element on line ${index + 1}`, elementsByExternalId);
            pendingElementFields = [];
        }
    }
...
export function parseLsdynaKeywordMesh(text) {
    const lines = normalizeImportText(text).split("\n");
    const nodesByExternalId = new Map();
    const elementsByExternalId = [];
    let section = "none";
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (section === "shell" || section === "solid") {
            ...
            parseExternalElementRecord(pendingElementFields.slice(0, 1).concat(pendingElementFields.slice(2)), expected, `LS-DYNA element on line ${index + 1}`, elementsByExternalId);
            pendingElementFields = [];
        }
    }
    if (pendingElementFields.length > 0) {
...
export function meshFromSerializable(value) {
...
function readArray(value, error) {
    if (!Array.isArray(value)) {
        throw new Error(error);
    }
    return value;
}
function readPoint(value, kind, nodeId) {
    if (!Array.isArray(value)) {
...
function readElement(value, elementId, kind, nodeCount) {
    if (!Array.isArray(value)) {
...
function validateElementNodeIds(element, elementId) {
...
function consumeNumericValues(lines, startIndex, count, sectionName) {
    const values = [];
...
function detectMeshImportFormat(text, fileName = "") {
    const normalized = normalizeImportText(text);
...
function parseMeshByFormat(text, format) {
    switch (format) {
        case "json":
            return parseMeshJSON(text);
        case "vtk":
...
function readAbaqusElementTypeSpec(line) {
...
function inferConnectivityNodeCountFromFieldCount(fieldCount, label) {
    if (fieldCount === 5) {
        return 4;
    }
    if (fieldCount === 9) {
...
function parseExternalElementRecord(fields, nodeCount, label, elements) {
...
function buildMeshFromExternalRecords(nodesByExternalId, externalElements, label) {
...
function splitDelimitedRecord(line) {
    return line
        .trim()
        .split(/[,\s]+/)
        .map((part) => part.trim())
...
function parsePointFields(fields, label, minDimension, maxDimension) {
...
function matchXmlSection(text, tagName, errorMessage) {
...
function matchVtuDataArray(text, predicate, label) {
...
function readRequiredIntegerAttribute(attributes, name, label) {
...
function parseVtuNumericDataArray(array, expectedCount, label) {
