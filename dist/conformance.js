...
export function checkNoHangingNodes(mesh, tolerance = 1e-9) {
...
function collectUnmatchedBoundaries(mesh, kind) {
    ...
    return [...recordsByKey.values()].filter((record) => record !== undefined);
}
function compareNearbyUnmatchedBoundaries(mesh, unmatched, kind, tolerance, hanging, seen) {
    ...
    for (const index of sortedIndices) {
        ...
        const stillActive = active.filter((otherIndex) => unmatched[otherIndex].bbox.max[sweepAxis] >= boundaryMin - tolerance);
        active.length = 0;
...
function boundaryPoints(mesh, nodeIds) {
...
function chooseSweepAxis(unmatched) {
    let bestAxis = 0;
...
function boundaryBoxesOverlap(a, b, tolerance) {
    for (let axis = 0; axis < 3; axis += 1) {
...
function addEdgeIssues(a, b, tolerance, hanging, seen) {
    const ap = a.points;
    const bp = b.points;
...
function addFaceIssues(a, b, tolerance, hanging, seen) {
    const ap = a.points;
    const bp = b.points;
...
function addIssue(kind, a, b, _tolerance, hanging, seen, nodeId) {
...
function zipNodes(boundary) {
    const points = boundary.points;
    return boundary.nodeIds.map((nodeId, index) => [nodeId, points[index]]);
}
function collinearSegments(a0, a1, b0, b1, tolerance) {
...
function segmentsOverlap1d(a0, a1, b0, b1, tolerance) {
    const axis = dominantAxis(a0, a1);
...
function coplanarFaces(a, b, tolerance) {
...
function pointInQuad(point, quad, tolerance) {
...
function pointInPolygon2d(point, polygon, tolerance) {
    let inside = false;
...
function axisAlignedBoxesOverlap(a, b, tolerance) {
...
function averagePoint(points) {
    ...
    return Array.from({ length: dimension }, (_, d) => points.reduce((total, point) => total + (point[d] ?? 0), 0) / points.length);
}
...
function dominantNormalAxis(normal) {
