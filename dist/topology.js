export const Q1_EDGES = [
    [1, 2],
    [2, 3],
    [3, 4],
    [4, 1]
];
export const H1_FACES = [
    [1, 2, 3, 4],
    [5, 6, 7, 8],
    [1, 5, 6, 2],
    [4, 8, 7, 3],
...
export function elementKindFromNodeCount(nodeCount) {
    if (nodeCount === 4) {
        return "Q1";
    }
    if (nodeCount === 8) {
...
export function boundariesOf(element, kind) {
    const templates = kind === "Q1" ? Q1_EDGES : H1_FACES;
    return templates.map((boundary) => boundary.map((localId) => getNode(element, localId)));
}
export function boundaryKey(boundary) {
    return [...boundary].sort((a, b) => a - b).join(":");
}
export function sameNodeSet(a, b) {
    return boundaryKey(a) === boundaryKey(b);
}
export function h1Permutations(element) {
    if (element.length !== 8) {
        throw new Error("h1Permutations requires an 8-node H1/Hex element");
    }
    return H1_FACE_PERMUTATIONS.map((permutation) => permutation.map((localId) => getNode(element, localId)));
}
export function alignHexFaceToBottom(element, face) {
    const match = h1Permutations(element).find((permutation) => sameNodeSet(permutation.slice(0, 4), face));
    if (!match) {
        throw new Error(`face [${face.join(", ")}] is not part of element [${element.join(", ")}]`);
    }
    return match;
}
export function alignHexEdgeToVertical(element, edge) {
    const match = h1Permutations(element).find((permutation) => sameNodeSet([permutation[0], permutation[4]], edge));
    if (!match) {
        throw new Error(`edge [${edge.join(", ")}] is not part of element [${element.join(", ")}]`);
    }
    return match;
}
export function alignQuadEdgeToFirst(element, edge) {
    if (element.length !== 4) {
...
export function alignQuadCornerToSecond(element, cornerNodeId) {
    if (element.length !== 4) {
...
export function sharedNodes(a, b) {
    const bSet = new Set(b);
    return a.filter((nodeId) => bSet.has(nodeId));
}
function getNode(element, localId) {
